...
-- Insercion de datos en la tabla Venta y DetalleVenta (Actividad Baja)
...
WHILE @i < 10000 -- 10,000 venta para actividad baja
BEGIN
    SET @ClienteID = FLOOR(RAND() * 10000) + 1; -- Asumimos 10,000 clientes
    --SET @FechaVenta = DATEADD(DAY, FLOOR(RAND() * 31), '2023-01-01'); -- Enero (Actividad Baja)
    --SET @FechaVenta = DATEADD(DAY, FLOOR(RAND() * 31), '2023-01-01'); -- Enero (Actividad Baja)
